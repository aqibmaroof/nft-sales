...
function CheckoutModal(props) {
  ...
  useEffect(async () => {
    ...
    if (props.metaMaskAddress) {
      ...
      if (props.isPublicSaleOpen) {
        ...
        axios
          .get(
            `${baseUrl}/blind-box/get-user-limit?address=${props.metaMaskAddress}&blindBoxId=17`
          )
          .then(async (res) => {
  ...
  const getPlotRates = async () => {
    ...
    if (collDataPrice !== undefined) {
      ...
      if (maxMint && totalSupply) {
        if (
          parseInt(maxMint) <=
          parseInt(totalSupply) + parseInt(reservedSupply)
        ) {
          setIsSoldOut(true);
        } else {
          setIsSoldOut(false);
  ...
  const getUserProof = async () => {
    ...
    let proof = await axios.get(
      `${baseUrl}/blind-box/get-proof-hash?address=${props.metaMaskAddress}&limit=${limit}&type=${allowedCategories[selectedSaleTypeIndex].id}&blindBoxId=17`
    );
    return proof;
  };
    ...
    web3.eth.getAccounts().then(async (acc) => {
      ...
      if (parseFloat(balance) < parseFloat(totalPrice)) {
        ...
        if (user_proof.status !== 200 || user_proof.data?.hexProof === 0) {
          setLoaderFor("");
          props.setTransactionInProgress(false);
          showNotitication(`You are not Allowlist to buy this.`, "error");
          return;
        }
        if (
  ...
  const preOrderAstroboyNFT = (acc, user_proof, whitelistId) => {
    let web3 = new Web3(props.provider);
    let LandContract = new web3.eth.Contract(deemoAbi, deemoAddress);
    console.log(user_proof, plotData.units);
    LandContract.methods
      .preOrder(user_proof, plotData.units, whitelistId)
      .send({
        from: acc[0],
        value: web3.utils.toHex(
          web3.utils.toWei(totalPrice.toString(), "ether")
  ...
  const claimAstroboyNFT = () => {
    ...
    web3.eth.getAccounts().then(async (acc) => {
      setLoaderFor("Claim");
      LandContract.methods
        ._claimWithRemainingPrice(acc[0])
        .call()
        .then((price) => {
          ...
          let amount =
            networkType === "testnet"
              ? parseFloat(web3.utils.fromWei(price.toString()))
              : parseInt(web3.utils.fromWei(price.toString()));
          let gas = "10000";
