...
import CheckoutModal from "./checkoutModalBreakDownNftCollection";
...
import { mainTypes } from "./BreakDownNftCollectionConfig";
...
function BlindBoxOffChain(props) {
  ...
  const setCurrentRunningSale = async () => {
    ...
    if (ongoingSale && ongoingSaleObj !== {}) {
      setCurrentSaleObj(ongoingSaleObj);
    } else if (saleToStart && saleToStartObj !== {}) {
      ...
      if (collDataPrice === "0" || collDataPrice === "1") {
        ...
        if (
          ...
          saleToStartObj = {
            ...
            name: "Public Sale & Claim Preordered NFTs",
            // name : "Public Sale &  Claim Preordered NFTs"
          };
        }

        if (
          parseInt(categoryTypesData[1]._startTime) * 1000 >
            new Date().getTime() &&
          parseInt(categoryTypesData[1]._endTime) * 1000 > new Date().getTime()
        ) {
          console.log("first");
          ongoingSale = true;
          ongoingSaleObj = {
            ...mainTypes[1],
            startTime: parseInt(categoryTypesData[1]._startTime) * 1000,
            endTime: parseInt(categoryTypesData[1]._endTime) * 1000,
            name: " Allowlist Sale & Claim Pre-ordered NFTs",
          };
        }
        ...
        if (
          ...
          parseInt(categoryTypesData[5]._startTime) * 1000 <
            new Date().getTime() &&
          parseInt(categoryTypesData[5]._endTime) * 1000 > new Date().getTime()
        ) {
          ongoingSale = true;
          ongoingSaleObj = {
            ...mainTypes[5],
            startTime: parseInt(categoryTypesData[5]._startTime) * 1000,
            endTime: parseInt(categoryTypesData[5]._endTime) * 1000,
  ...
  const getMaxMint = async () => {
    ...
    let perBundleNfts = await landContract.methods
      ...
      .call();
    console.log("sale Stats", maxMint, totalSupply, reservedSupply);
  ...
  const genericOwnedNFTMoralis = async (
    ...
    for (let i = 0; i < blockChainConfig.length; i++) {
      if (blockChainConfig[i].key === selectedNFTChain) {
        ocBBAdd = blockChainConfig[i].breakDownConfig.add;
      }
    }
                  ...
                  <img
                    ...
                    src={
                      selectedBlindBox?.bannerImage
                        ? selectedBlindBox?.bannerImage
                        : "https://ik.imagekit.io/xanalia/CollectionMainData/Banner_11.jpg"
                    }
                      ...
                      >
                        <img
                          ...
                          src={
                            selectedBlindBox?.packVideo
                              ? selectedBlindBox?.packVideo
                              : `https://ik.imagekit.io/xanalia/CollectionMainData/bdPackImageCollection.jpg`
                          }
                        />
                        {/* <video
                          autoPlay={true}
                                ...
                                <div className="text-white">
                                  ...
                                  currentSaleObj?.id === 4 ? (
                                    <>
                                      ...
                                      {currentSaleObj?.name === " Allowlist"
                                        ? " Allowlist Sale & Claim Pre-ordered NFTs"
                                        : currentSaleObj?.name}
                                    </>
                                  ) : (
                                    <>
                                      Time Left to End the
                                      {currentSaleObj?.name} Sale
                                    </>
                                  )}
                                </div>
