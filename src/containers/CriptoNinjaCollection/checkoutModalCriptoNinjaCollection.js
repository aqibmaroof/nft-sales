...
import { mainTypes, Options } from "./CriptoNinjaCollectionConfig";
import CountDown from "../../component/CountDown/countDown";
import Disclaimer from "./disclaimerModal";
import "./CriptoNinjaCollection.scss";
...
function CheckoutModal(props) {
  ...
  useEffect(async () => {
    ...
    if (props.metaMaskAddress) {
      ...
      if (props.isPublicSaleOpen) {
        ...
        axios
          .get(
            `${baseUrl}/blind-box/get-user-limit?address=${props.metaMaskAddress}&blindBoxId=15`
          )
          .then(async (res) => {
  ...
  const setContractAddress = () => {
    for (let i = 0; i < blockChainConfig.length; i++) {
      if (blockChainConfig[i].key === nftChain) {
        ...
        deemoAbi = blockChainConfig[i].criptoNinjaConfig.abi;
        deemoAddress = blockChainConfig[i].criptoNinjaConfig.add;
      }
    }
  };
  ...
  const getPlotRates = async () => {
    ...
    if (collDataPrice !== undefined) {
      ...
      let reservedSupply = await landContract.methods
        .reservedNFT(plotData.type)
        .call();
  ...
  const getUserProof = async () => {
    ...
    let proof = await axios.get(
      `${baseUrl}/blind-box/get-proof-hash?address=${props.metaMaskAddress}&limit=${limit}&type=${allowedCategories[selectedSaleTypeIndex].id}&blindBoxId=15`
    );
    return proof;
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    if (parseInt(plotData["units"]) > 30) {
      showNotitication(
        `You can able to purchase maximum of 30 Nfts in a transaction`,
        "info"
      );
      return;
    }
  ...
  const preOrderAstroboyNFT = (acc, user_proof) => {
    ...
    LandContract.methods
      .preOrderCryptoNFT(user_proof, plotData.units)
      .send({
        from: acc[0],
                        ...
                        {new Date().getTime() >= type.startTime ? (
                              ...
                              <FormGroup>
                                ...
                                <div className="opt-container-time  align-self-right">
                                  <FormLabel className="text-white text mb-0">
                                    Time Left to End the {props.saleType} Sale
                                  </FormLabel>
                                  <CountDown
                          ...
                          <>
                            <div className="CountDownClass">
                              <h4>
                                {type.id !== 3
                                  ? `Time Left to start ${props.saleType} Sale`
                                  : `Time Left to start  ${props.saleType}`}
                              </h4>
