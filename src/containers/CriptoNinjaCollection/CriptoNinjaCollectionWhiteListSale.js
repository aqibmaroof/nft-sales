...
import CheckoutModal from "./checkoutModalCriptoNinjaCollection";
...
import { mainTypes } from "./CriptoNinjaCollectionConfig";
    ...
    for (let i = 1; i < mainTypes.length; i++) {
      if (
        ...
        if (saleToStartObj.name === "Whitelist") {
          saleToStartObj.name = "Allowlist Sale";
        } else {
        }
        break;
      }
    ...
    for (let i = 1; i < mainTypes.length; i++) {
      if (
        ...
        if (ongoingSaleObj.name === "Whitelist") {
          ongoingSaleObj.name = "Allowlist Sale";
        } else {
        }
        break;
      }
  ...
  const genericOwnedNFTMoralis = async (
    ...
    for (let i = 0; i < blockChainConfig.length; i++) {
      if (blockChainConfig[i].key === selectedNFTChain) {
        ocBBAdd = blockChainConfig[i].criptoNinjaConfig.add;
      }
    }
                      ...
                      >
                        <img
                          ...
                          src={
                            selectedBlindBox?.packVideo
                              ? selectedBlindBox?.packVideo
                              : `https://ik.imagekit.io/xanalia/CollectionMainData/penpenz_pack_image.jpg`
                          }
                        />
                        {/* <video
                          autoPlay={true}
                            ...
                            <>
                              <div className="padding-t1">
                                <div className="text-white">
                                  Time Left to End the {currentSaleObj.name} Sale{" "}
                                </div>
