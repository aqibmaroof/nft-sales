...
import { mainTypes, Options } from "./corporateCollectionConfig";
...
function CheckoutModal(props) {
  ...
  useEffect(async () => {
    ...
    if (props.metaMaskAddress) {
      ...
      if (props.isPublicSaleOpen) {
        ...
        axios
          .get(
            `${baseUrl}/blind-box/get-user-limit?address=${props.metaMaskAddress}&blindBoxId=10`
          )
          .then(async (res) => {
  ...
  const getUserProof = async () => {
    ...
    let proof = await axios.get(
      `${baseUrl}/blind-box/get-proof-hash?address=${props.metaMaskAddress}&limit=${limit}&type=${allowedCategories[selectedSaleTypeIndex].id}&blindBoxId=10`
    );
    return proof;
  };
  ...
  const preOrderAstroboyNFT = (acc, user_proof) => {
    ...
    LandContract.methods
      .preOrderTshirtNFT(user_proof, plotData.units)
      .send({
        from: acc[0],
        value: web3.utils.toHex(
          web3.utils.toWei(totalPrice.toString(), "ether")
                            ...
                            type.id !== 3 ? (
                              <FormGroup>
                                ...
                                <div className="opt-container-time  align-self-right">
                                  ...
                                  Time Left to End the {props.saleType}
                                </FormLabel>
                                  <CountDown
